...
const App = () => {
  ...
  function utxoImage(utxo, style) {
    console.log("test", ordinalsImageUrl(inscriptionUtxosByUtxo[`${utxo.txid}:${utxo.vout}`]))
    return (
      <img
        alt=""
        src={
        ...
        {ownedUtxos.length === 0 ? (
          <>
            <div>
              ...
              <br />
              Consider minting an{" "}
              <a href="https://astralbabes.ai" target="_blank">
                astral babe
              </a>
            </div>
          </>
          ...
          <>
            ...
            <Container className="d-flex flex-wrap">
              {ownedUtxos
                .filter((utxo) => Boolean(utxo))
                .map((utxo) => (
                  <UTXO key={utxo.txid} utxo={utxo} inscriptionUtxosByUtxo={inscriptionUtxosByUtxo} />
                ))}
            </Container>
          </>
        )}
  ...
  return (
    <>
      <Navbar bg="dark" variant="dark" className="pt-5 pb-5">
        <Container>
          <Navbar.Brand className="flex-row">Nos-FT</Navbar.Brand>
        ...
        {nostrPublicKey ? (
          <div>
            <Button
              ...
              onClick={() => setShowReceiveAddressModal(true)}
            >
          ...
          <>
            <p>Welcome, Degen</p>
            <br />
            <Button variant="primary" className="mx-3 shadowed-orange-small" onClick={() => connectWallet()}>
              Connect Wallet
            </Button>
          </>
        )}
      ...
      >
        <Modal.Header closeButton className="p-4">
          <Modal.Title>
            {shortenStr(currentUtxo && `${currentUtxo.txid}:${currentUtxo.vout}`)}:
            {currentUtxo && currentUtxo.vout}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body className="modal-body p-4">
          <div>{currentUtxo && utxoImage(currentUtxo, { width: "60%" })}</div>
          <p>
            <b>Utxo:</b>{" "}
            <a href={currentUtxo && ordinalsUrl(currentUtxo)} target="_blank">
      ...
      >
        <Modal.Header closeButton className="p-4">
          <Modal.Title>
            Send {shortenStr(currentUtxo && `${currentUtxo.txid}:${currentUtxo.vout}`)}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body className="modal-body p-4">
      ...
      <Modal show={showSelectFeeRateModal} onHide={() => setShowSelectFeeRateModal(false)} className="py-5">
        <Modal.Header closeButton className="p-4">
          <Modal.Title>
            Sending {shortenStr(currentUtxo && `${currentUtxo.txid}:${currentUtxo.vout}`)}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body className="modal-body p-4">
          {currentUtxo && utxoImage(currentUtxo, { width: "60%" })}
